...
def format_tz_set(morning_time: time) -> str:
    """Сообщение после сохранения часового пояса; время утра берётся из настроек пользователя."""
    return f"Часовой пояс сохранён. Завтра в {morning_time.strftime('%H:%M')} пришлю запрос плана."
...
PLAN_DELETE_CONFIRM = "Вы уверены, что хотите удалить план на {date}? Это действие нельзя отменить."
...
TEST_MORNING_SENT = "Тест пройден: утреннее сообщение доставлено."
TEST_EVENING_SENT = "Тест пройден: вечернее сообщение доставлено."
TEST_DELIVERY_ERROR = "Ошибка доставки: {error}"
COMMANDS_OVERVIEW = (
    "Команды управления:\n"
    "/plan - добавить план на сегодня вручную\n"
    "/today - посмотреть план на сегодня\n"
    "/delete_plan - удалить план на сегодня\n"
    "/history YYYY-MM - история планов за месяц\n"
    "/stats - общая статистика\n"
    "/settings - текущие настройки\n"
    "/reminders - список кастомных напоминаний\n"
    "/reminder_add - добавить новое напоминание\n"
    "/time - время на сервере бота и твой часовой пояс\n"
    "/me - твой Telegram ID и запись в БД (для диагностики)\n"
    ...
    "/check_cron - проверить настройки и время до следующего уведомления\n"
    "/test_morning - тест утреннего уведомления\n"
    "/test_evening - тест вечернего уведомления\n"
    "/retry_morning - отправить утреннее ещё раз сегодня (снять блокировку)\n"
    "/retry_evening - отправить вечернее ещё раз сегодня (снять блокировку)\n"
